...
# -------------------------------------------------- legacy manifests & names

def test_a_legacy_json_manifest_still_loads(tmp_path):
    """Kept only so an image with baked manifests keeps working while packages
    roll out. Goes away with the manifest generator."""
    import json
    d = tmp_path / "legacy" / "1.0"
    d.mkdir(parents=True)
    (d / toolchains.LEGACY_MANIFEST_NAME).write_text(json.dumps({
        "name": "legacy-tc", "arch": "xtensa", "version": "1.0",
...
def test_a_package_shadows_a_legacy_manifest_of_the_same_name(tmp_path):
    import json
    write_package(tmp_path, "pkg", body=MINIMAL.replace("name: fake-elf", "name: shared"))
    d = tmp_path / "legacy"
    d.mkdir()
    (d / toolchains.LEGACY_MANIFEST_NAME).write_text(json.dumps({
        "name": "shared", "arch": "old", "version": "0.1",
        "exe": "/opt/old/gdb", "converter": "passthrough",
    }))
    assert toolchains.get("shared").arch == "fakearch", \
        "the package must win, or mounting one could not replace a baked manifest"
